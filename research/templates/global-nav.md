...
7. **Mobile** (≤1024px): hamburger `#brxe-fsrjct` appears; offcanvas accordion duplicates the ENTIRE menu tree (all ~30 links), then at the very bottom: **"Ring en expert" → tel:+46102657979** (teal button, phone icon) + a bare **"5.0"** Google rating chip. The tel CTA sits after the full accordion stack in DOM order.
...
**0–5 s:** Lands from Google on a service page. Header: logo, "Gratis rådgivning" pill, hamburger. To *call* — the faster, higher-intent path for "elfel i huset" searchers — she must open the hamburger, scroll past a ~30-item accordion, and find "Ring en expert" at the bottom. The visible one-tap conversion is the form path only.
...
**GLOB-04 · P1 · Phone path is buried on mobile despite phone being conversion path #1.**
Desktop header: no phone at all. Mobile: tel CTA "Ring en expert" exists only at the bottom of the offcanvas (hamburger → scroll past ~30-link accordion). The sticky header spends its one CTA slot on the *form* path. GA4 shows 2 phone clicks vs 0 form leads from paid — the phone is the path that actually produces. Evidence: Fitts's law / thumb-zone reachability; Unbounce home-services benchmark (urgent/repair intent converts by phone); two-conversion doctrine. Mobile note: this IS the mobile finding; desktop should also carry a visible number for the 55–65 cohort who dial from memory. Priority: 325 × 3 (header=first-viewport) × 2 = **1950**.
...
(a) E-mail is Cloudflare-obfuscated — with JS off it literally renders "[email protected]" (verified in fetched text). (b) "Få en kostnadsfri konsultation!" is an H3 *heading*, styled like the CTA language used in buttons elsewhere — looks tappable, is not. (c) Prefooter column headings (Elinstallation, Laddboxar, Batterilagring, Områden, Lösningar) are unlinked H3s — each should link its pillar (/elservice/, /laddboxar/, /solcellsbatterier/, områden-hub, /foretag/). Mobile: (b) is a thumb-trap. Priority: 325 × 1 × 1 = 325.
...
| 9 | Mobile offcanvas: **"Ring en expert" + rating moved to TOP of panel** (above accordion), tel bar persists | reachability without full-scroll | modified |
